...
export const getScannerStatus = functions.https.onCall(async (data, context) => {
    const timestamp = new Date().toISOString();
    console.log(`🔍 [${timestamp}] getScannerStatus called with data:`, JSON.stringify(data, null, 2));
    ...
    try {
        ...
        console.log(`✅ [${timestamp}] getScannerStatus result:`, {
        ...
        console.error(`💥 [${timestamp}] getScannerStatus error:`, {
            error: error instanceof Error ? error.message : String(error),
            errorStack: error instanceof Error ? error.stack : 'No stack'
        });
        throw new functions.https.HttpsError('internal', `Scanner status error: ${error instanceof Error ? error.message : String(error)}`);
    }
});
...
export const triggerRepositoryScan = functions.https.onCall(async (data, context) => {
    const timestamp = new Date().toISOString();
    console.log(`🔍 [${timestamp}] triggerRepositoryScan called with data:`, JSON.stringify(data, null, 2));

    const { projectId, scanType = 'medium', repoUrl, githubToken } = data;
    ...
    try {
        ...
        if (!scannerManager) {
            ...
            throw new functions.https.HttpsError('internal', 'Scanner manager not available');
        }
        ...
        console.log(`✅ [${timestamp}] triggerRepositoryScan completed:`, {
            projectId,
            scanType,
            scanTriggered: true,
        ...
        console.error(`💥 [${timestamp}] triggerRepositoryScan error:`, {
            error: error instanceof Error ? error.message : String(error),
            errorStack: error instanceof Error ? error.stack : 'No stack',
            projectId
        });
        throw new functions.https.HttpsError('internal', `Repository scan error: ${error instanceof Error ? error.message : String(error)}`);
    }
});

// Get the latest scan results for a project
export const getScanResults = functions.https.onCall(async (data, context) => {
    const timestamp = new Date().toISOString();
    console.log(`📊 [${timestamp}] getScanResults called with data:`, JSON.stringify(data, null, 2));
    ...
    try {
        ...
        console.log(`✅ [${timestamp}] getScanResults completed:`, {
            projectId,
        ...
        console.error(`💥 [${timestamp}] getScanResults error:`, {
            error: error instanceof Error ? error.message : String(error),
            errorStack: error instanceof Error ? error.stack : 'No stack',
            projectId
        });
        throw new functions.https.HttpsError('internal', `Get scan results error: ${error instanceof Error ? error.message : String(error)}`);
    }
});
